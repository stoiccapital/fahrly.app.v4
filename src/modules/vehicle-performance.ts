...
export type VehicleStatus = 'Active' | 'In workshop' | 'In reserve' | 'Retired';
export type MaintenanceStatus = 'OK' | 'Due soon' | 'Overdue' | 'In workshop';
export type VehicleType = 'Car' | 'Van' | 'Truck';
export type SortField = 'vpi' | 'costPerKm' | 'safety' | 'none';
...
export interface VehiclePerformance {
  ...
  vpi: number; // 0–100
  utilization: number; // 0–1 (e.g. 0.71 = 71%)
  utilizationKm: number;
...
// Mock data
const mockVehicles: VehiclePerformance[] = [
  {
    id: '1',
    name: 'VW Caddy #12',
    plate: 'ZH 123 456',
...
// State
let vehicles: VehiclePerformance[] = [...mockVehicles];
let filteredVehicles: VehiclePerformance[] = [...mockVehicles];
let selectedVehicle: VehiclePerformance | null = null;
let startDate: string = '';
let endDate: string = '';
let currentSortField: SortField = 'none';
let currentSortDirection: SortDirection = 'asc';
let currentStatusFilter: VehicleStatus | 'All' = 'All';
let currentTypeFilter: VehicleType | 'All' = 'All';
let searchQuery: string = '';

// Manual data entry arrays
let usageDaily: UsageDaily[] = [];
let fuelLogs: FuelLog[] = [];
let maintenanceEvents: MaintenanceEvent[] = [];
let incidents: IncidentEvent[] = [];
...
// Utility functions
function getVPIColorClass(vpi: number): string {
  if (vpi >= 80) return 'vf-badge--green';
  if (vpi >= 60) return 'vf-badge--yellow';
  return 'vf-badge--red';
}

function getSafetyColorClass(grade: SafetyGrade): string {
  if (grade === 'A') return 'vf-badge--green';
  if (grade === 'B') return 'vf-badge--yellow';
  return 'vf-badge--red';
}

function getMaintenanceColorClass(status: MaintenanceStatus): string {
  if (status === 'OK') return 'vf-badge--green';
  if (status === 'Due soon') return 'vf-badge--yellow';
  return 'vf-badge--red';
}
    ...
    filtered.sort((a, b) => {
      ...
      switch (currentSortField) {
        ...
        case 'costPerKm':
          ...
          break;
        case 'safety':
          const gradeOrder: Record<SafetyGrade, number> = { 'A': 4, 'B': 3, 'C': 2, 'D': 1 };
...
function renderHeader(): void {
  ...
  startDateInput = document.getElementById('vf-start-date') as HTMLInputElement;
  endDateInput = document.getElementById('vf-end-date') as HTMLInputElement;
  ...
  // Add Data dropdown
  const addDataBtn = document.getElementById('vf-add-data-btn') as HTMLElement;
  const dropdown = document.getElementById('vf-add-data-dropdown') as HTMLElement;
  ...
  dropdownItems.forEach(item => {
    item.addEventListener('click', (e) => {
      const formType = (e.target as HTMLElement).dataset.form;
      dropdown.classList.remove('vf-dropdown-menu--open');
      if (formType) {
        openModal(formType);
      }
...
function renderFilters(): void {
  ...
  searchInput = document.getElementById('vf-search-input') as HTMLInputElement;
  statusFilter = document.getElementById('vf-status-filter') as HTMLSelectElement;
  typeFilter = document.getElementById('vf-type-filter') as HTMLSelectElement;
  ...
  statusFilter.addEventListener('change', (e) => {
    currentStatusFilter = (e.target as HTMLSelectElement).value as VehicleStatus | 'All';
    applyFiltersAndSorting();
  });

  typeFilter.addEventListener('change', (e) => {
    currentTypeFilter = (e.target as HTMLSelectElement).value as VehicleType | 'All';
    applyFiltersAndSorting();
  });
  ...
  sortButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
      const field = (e.target as HTMLElement).dataset.sort as SortField;
...
function updateSortButtons(): void {
  ...
  sortButtons.forEach(btn => {
    ...
    (btn as HTMLElement).classList.remove('vf-sort-btn--active', 'vf-sort-btn--asc', 'vf-sort-btn--desc');
    if (currentSortField === field) {
      (btn as HTMLElement).classList.add('vf-sort-btn--active', `vf-sort-btn--${currentSortDirection}`);
    }
  });
}
...
function renderMainLayout(): void {
  ...
  tableBody = document.getElementById('vf-table-body') as HTMLElement;
  detailPanel = document.getElementById('vf-detail-panel') as HTMLElement;
...
function renderDetailPanel(): void {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', (e) => {
      ...
      (e.target as HTMLElement).classList.add('vf-tab--active');
...
function renderTabContent(tab: string): void {
  if (!selectedVehicle || !detailPanel) return;
  ...
  switch (tab) {
    ...
    case 'safety':
      const safetyExplanation: Record<SafetyGrade, string> = {
        'A': 'Excellent',
        'B': 'Good',
        'C': 'Risky',
        'D': 'Critical'
...
function openModal(formType: string): void {
  ...
  const modal = modalOverlay.querySelector('.vf-modal') as HTMLElement;
...
function renderFuelForm(): string {
  return `
    <div class="vf-modal-header">
...
function renderMaintenanceForm(): string {
  return `
    <div class="vf-modal-header">
...
function renderIncidentForm(): string {
  return `
    <div class="vf-modal-header">
...
function handleFormSubmit(formType: string, form: HTMLFormElement): void {
  let vehicleId: string = '';
  
  switch (formType) {
    case 'usage':
      const usageData: UsageDaily = {
        vehicleId: (form.querySelector('#usage-vehicle') as HTMLSelectElement).value,
        date: (form.querySelector('#usage-date') as HTMLInputElement).value,
        odometerKm: parseFloat((form.querySelector('#usage-odometer') as HTMLInputElement).value),
        drivingHours: parseFloat((form.querySelector('#usage-driving-hours') as HTMLInputElement).value),
        idleHours: parseFloat((form.querySelector('#usage-idle-hours') as HTMLInputElement).value),
        jobsCount: (form.querySelector('#usage-jobs') as HTMLInputElement).value ? parseInt((form.querySelector('#usage-jobs') as HTMLInputElement).value) : undefined
      };
    ...
    case 'fuel':
      const fuelData: FuelLog = {
        vehicleId: (form.querySelector('#fuel-vehicle') as HTMLSelectElement).value,
        dateTime: (form.querySelector('#fuel-datetime') as HTMLInputElement).value,
        liters: parseFloat((form.querySelector('#fuel-liters') as HTMLInputElement).value),
        totalCost: parseFloat((form.querySelector('#fuel-cost') as HTMLInputElement).value),
        odometerKm: parseFloat((form.querySelector('#fuel-odometer') as HTMLInputElement).value),
        notes: (form.querySelector('#fuel-notes') as HTMLTextAreaElement).value || undefined
      };
    ...
    case 'maintenance':
      const maintenanceData: MaintenanceEvent = {
        vehicleId: (form.querySelector('#maintenance-vehicle') as HTMLSelectElement).value,
        type: (form.querySelector('#maintenance-type') as HTMLSelectElement).value,
        cost: parseFloat((form.querySelector('#maintenance-cost') as HTMLInputElement).value),
        dateStart: (form.querySelector('#maintenance-date-start') as HTMLInputElement).value,
        dateEnd: (form.querySelector('#maintenance-date-end') as HTMLInputElement).value || undefined,
        notes: (form.querySelector('#maintenance-notes') as HTMLTextAreaElement).value || undefined
      };
    ...
    case 'incident':
      const incidentData: IncidentEvent = {
        vehicleId: (form.querySelector('#incident-vehicle') as HTMLSelectElement).value,
        driverId: (form.querySelector('#incident-driver') as HTMLInputElement).value || undefined,
        date: (form.querySelector('#incident-date') as HTMLInputElement).value,
        description: (form.querySelector('#incident-description') as HTMLTextAreaElement).value,
        damageCost: parseFloat((form.querySelector('#incident-damage-cost') as HTMLInputElement).value),
        insuranceCovered: (form.querySelector('#incident-insurance') as HTMLInputElement).value ? parseFloat((form.querySelector('#incident-insurance') as HTMLInputElement).value) : undefined,
        notes: (form.querySelector('#incident-notes') as HTMLTextAreaElement).value || undefined
      };
...
function recalculateVehiclePerformance(vehicleId: string): void {
  ...
  const relevantUsage = usageDaily.filter(u => 
  ...
  const relevantFuelLogs = fuelLogs.filter(f => 
  ...
  const relevantMaintenance = maintenanceEvents.filter(m => 
  ...
  let safetyGrade: SafetyGrade = 'A';
  if (relevantIncidents.length >= 3) safetyGrade = 'D';
  else if (relevantIncidents.length >= 2) safetyGrade = 'C';
  else if (relevantIncidents.length >= 1) safetyGrade = 'B';
  ...
  if (daysUntilService < 0) maintenanceStatus = 'Overdue';
  else if (daysUntilService < 7) maintenanceStatus = 'Due soon';
...
function selectVehicle(vehicle: VehiclePerformance): void {
  ...
  const selectedRow = Array.from(rows).find(row => 
    row.querySelector('.vf-vehicle-name')?.textContent === vehicle.name
  );
  if (selectedRow) {
...
export function initVehiclePerformanceModule(root: HTMLElement): void {
  rootElement = root;
